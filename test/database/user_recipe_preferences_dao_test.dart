...
void main() {
  ...
  group('getPreferencesForRecipe', () {
    test('returns null when no preferences exist', () async {
      final result = await db.userRecipePreferencesDao.getPreferencesForRecipe(
        'r1',
      );
  ...
  group('updatePreferences - create (no existing record)', () {
    test('creates record with isFavorite=true', () async {
      ...
      final prefs = await db.userRecipePreferencesDao.getPreferencesForRecipe(
        'r1',
      );
    ...
    test('creates record with custom water temperature', () async {
      await db.userRecipePreferencesDao.updatePreferences(
        'r1',
        customWaterTemp: 92.5,
      );

      final prefs = await db.userRecipePreferencesDao.getPreferencesForRecipe(
        'r1',
      );
      expect(prefs!.customWaterTemp, 92.5);
    });

    test(
      'creates record with sweetness and strength slider positions',
      () async {
        await db.userRecipePreferencesDao.updatePreferences(
          'r1',
  ...
  group('getFavoritePreferences', () {
    test('returns only favorited recipes', () async {
      ...
      final favorites = await db.userRecipePreferencesDao
          .getFavoritePreferences();

      expect(favorites.length, 2);
      expect(favorites.map((p) => p.recipeId), containsAll(['r1', 'r3']));
    });
