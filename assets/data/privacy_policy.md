...
- The app collects anonymous event data such as brew starts and completions, bean additions, bean review interactions (creating, editing, deleting, and translating reviews), recipe creation and sharing, collection interactions and sharing, screen views, roaster profile views and taps on a roaster's external links (such as their website or social media), taps that open our blog (only which in-app entry point you used), donation funnel interactions, onboarding and first-steps journey progress, notification engagement (which reminder types you switched on or off, whether you allowed or declined the notification permission prompt and on which screen you were asked, and which in-app reminders were scheduled, delivered, opened, or cancelled — never their content), brew diary usage such as filters and searches performed (never the search text), entry edits, and exports of your own brew notes (only which kind of export you chose and how many entries it covered, never the notes themselves), impressions of and interactions with in-app "moments" (small surprise-and-delight touches such as celebratory cards and seasonal banners), and which brewing-screen layout (classic or immersive) you use and when you change it and from which screen (Settings, the Preparation screen's advanced features menu, the one-time layout picker shown on the Play button, or a suggestion on the finish screen), and, if you choose to tell us why you switched back to the classic timer, your answer from a short fixed list of reasons such as "hard to read" or "I prefer the classic timer" (never free text). No personal information such as your name, email, or account ID is included in analytics events. New installs may also be automatically assigned one of the two brewing-screen layouts as their starting default, chosen at random, so we can compare how each performs; you can change your layout at any time from Settings or the Preparation screen.
